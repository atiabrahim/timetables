...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [theme, setThemeState] = useState<ThemeType>(() => (localStorage.getItem("scheduler_theme") as ThemeType) || "emerald");
  ...
  useEffect(() => {
    ...
    if (theme !== "emerald") root.classList.add(`theme-${theme}`);
    localStorage.setItem("scheduler_theme", theme);
  }, [theme]);

  useEffect(() => {
    const savedData = localStorage.getItem(STORAGE_KEY);
    if (savedData) try { importAllData(JSON.parse(savedData)); } catch (e) {}
    loadDataFromCloud(true);
  }, []);

  useEffect(() => {
    const dataToSave = { systemUsers, institution, employees, assignments, requirements, templateAssignments, dailyAssignments, departments, rooms, classes, subjects, periodConfigs, teacherConstraints, classConstraints, roomConstraints, periodTimings };
  ...
  const loadDataFromCloud = async (silent = false) => {
    try {
      const { data, error } = await supabase.from('app_sessions').select('data').eq('id', 'default_session').maybeSingle();
      if (error) throw error;
      if (data?.data) {
        importAllData(data.data);
        if (!silent) showSuccess(isRTL ? "تم استرداد البيانات بنجاح" : "Data loaded successfully");
      } else if (!silent) showError(isRTL ? "لا توجد بيانات محفوظة سحابياً" : "No cloud data found");
    } catch (err: any) {
      if (!silent) showError(isRTL ? `فشل الاسترداد: ${err.message}` : `Load failed: ${err.message}`);
    }
  };
  ...
  const saveAssignment = (date: string, periods: PeriodPart[], employeeIds: string[]) => {
    setDailyAssignments(prev => {
      ...
      const newAssignments = periods.map(period => ({ date, period, employeeIds: uniqueIds }));
  ...
  const getEffectiveAssignment = (dateStr: string, period: PeriodPart): string[] => {
    ...
    const timetableIds = assignments.filter(a => {
  ...
  const importAllData = (data: Partial<AppState>) => {
    ...
    if (data.periodConfigs?.length) setPeriodConfigs(data.periodConfigs);
  ...
  const loadDemoData = () => {
    ...
    const demoRooms = ["القاعة 1", "المخبر 1", "ورشة 1"];

    const demoConstraints: TeacherConstraint[] = [
      { employeeId: "emp-1", day: 2, period: "1", isAvailable: false },
      { employeeId: "emp-1", day: 2, period: "2", isAvailable: false },
      { employeeId: "emp-2", day: 4, period: "5", isAvailable: false }
    ];
    ...
    setRooms(demoRooms);
    setTeacherConstraints(demoConstraints);
  ...
  const login = (username: string, role: User["role"]) => {
    ...
    const userToLogin = found || { id: Math.random().toString(36).substr(2, 9), username, fullName: username, email: "", role, observation: "", isActive: true };
    setUser(userToLogin);
    localStorage.setItem("scheduler_user", JSON.stringify(userToLogin));
  };

  const logout = () => { setUser(null); localStorage.removeItem("scheduler_user"); };
