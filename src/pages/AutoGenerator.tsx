...
import { 
  ...
  Sparkles, ShieldCheck, Timer, FileDown, Trash2
} from "lucide-react";
...
const AutoGenerator = () => {
  ...
  const handleClearAll = () => {
    saveRequirements([]);
    showSuccess(isRTL ? "تم مسح كافة المتطلبات" : "All requirements cleared");
  };
            ...
            for (const period of rules.allowedPeriods) {
              if (checkHardConstraints(req, day, period, currentAssignments)) {
                ...
                if (score > highestScore) {
                  highestScore = score; bestDay = day; bestPeriod = period;
                }
              }
            }
          }
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-8 rounded-[2.5rem] border border-emerald-100 shadow-sm">
        ...
        <div className="flex gap-2">
          <Button 
            variant="outline"
            onClick={extractFromSchedule}
            className="h-14 px-6 rounded-2xl font-bold border-emerald-100 text-emerald-700 gap-2 hover:bg-emerald-50"
          >
            <FileDown size={20} />
            {isRTL ? "استخراج من الجدول" : "Extract from Table"}
          </Button>
          <Button 
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center px-4">
              ...
              {requirements.length > 0 && (
                <Button variant="ghost" size="sm" onClick={handleClearAll} className="text-red-500 hover:text-red-600 font-bold gap-1 text-[10px]">
                  <Trash2 size={12} /> {isRTL ? "مسح القائمة" : "Clear All"}
                </Button>
              )}
            </div>
            <RequirementTable 
