    ...
    <table className="w-full border-collapse border-2 border-slate-950 text-sm">
      <thead>
        <tr className="bg-slate-100 border-b-2 border-slate-950">
          ...
          <th className={cn("p-4 border-e-2 border-slate-950 font-black", isRTL ? "text-right" : "text-left")}>{isRTL ? "اسم المادة" : "Subject Name"}</th>
          <th className={cn("p-4 font-black", isRTL ? "text-right" : "text-left")}>{isRTL ? "التسمية بالإنجليزية" : "English Name"}</th>
        </tr>
      </thead>
      <tbody>
        {sortedAndFilteredSubjects.map((sub, idx) => (
          <tr key={sub.id} className="border-b border-slate-950">
            ...
            <td className="p-4 border-e border-slate-950 font-bold text-slate-900">{sub.name}</td>
            <td className="p-4 text-slate-700">{sub.nameEn || "---"}</td>
          </tr>
        ))}
      </tbody>
    </table>
