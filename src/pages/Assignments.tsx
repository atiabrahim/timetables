        ...
        {weekDays.map((day) => {
          ...
          return (
            <div key={dateStr} className="flex flex-col gap-3">
              <div className={cn("text-center p-2 rounded-xl font-medium transition-all duration-300", isToday ? "bg-emerald-600 text-white shadow-lg shadow-emerald-200" : "bg-white border border-slate-200 text-slate-800")}>
              ...
              <div className="space-y-3">
                {allPeriods.map(period => {
                  ...
                  return (
                    <button key={period} onClick={() => handleOpenAssignment(day, period)} className={cn("w-full text-start p-3 rounded-xl border transition-all duration-300 group", assignedCount > 0 ? "bg-white border-emerald-100 shadow-sm hover:border-emerald-400" : "bg-slate-50/50 border-dashed border-slate-300")}>
                      ...
                      {assignedCount > 0 ? (
                        <div className="space-y-1.5">
                          {assignedIds.slice(0, 3).map(id => {
                            ...
                            return <div key={id} className="text-xs text-slate-700 truncate font-medium flex items-center gap-1.5"><div className="h-1 w-1 rounded-full bg-slate-300"></div>{emp?.lastName}</div>;
                          })}
