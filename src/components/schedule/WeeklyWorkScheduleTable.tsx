          ...
          <Table className={cn("border-collapse border-spacing-0 table-fixed", isPrint ? "w-full border-2 border-black" : "w-full min-w-[1000px]")}>
            ...
            <TableHeader>
              <TableRow className={cn(isPrint ? "bg-slate-100/50 border-b-2 border-black h-5" : "bg-emerald-50/50 h-7")}>
                <TableHead className={cn("font-black border text-center", isPrint ? "text-[7px] p-0.5 border-black text-black leading-none" : "text-xs p-1 border-emerald-100 text-emerald-900")}>{isRTL ? "اليوم" : "Day"}</TableHead>
                <TableHead className={cn("font-black border text-center", isPrint ? "text-[7px] p-0.5 border-black text-black leading-none" : "text-xs p-1 border-emerald-100 text-emerald-900")}>{isRTL ? "الحصة" : "Period"}</TableHead>
                {sortedEmployees.map(emp => (
                ...
                <TableHead className={cn("font-black border text-center", isPrint ? "text-[7px] p-0.5 border-black text-black leading-none" : "text-[10px] p-1 border-emerald-100 text-emerald-900")}>{isRTL ? "کلي" : "Sum"}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {DAYS.map(day => {
                ...
                return activePeriods.map((p, pIdx) => (
                  <TableRow key={`${day.id}-${p}`} className={cn("group transition-colors duration-150", isPrint ? "h-5 border-b border-black" : "h-8 hover:bg-emerald-50/30")}>
                    ...
                    <TableCell className={cn("font-bold border bg-white text-center leading-none", isPrint ? "text-[6.5px] p-0.5 border-black text-black" : "text-[9px] p-1 border-emerald-100 text-slate-500")}>
                    ...
                    <TableCell className={cn("text-center border bg-slate-50/30 font-black leading-none", isPrint ? "text-[7px] border-black text-black" : "text-[10px] border-emerald-100 text-emerald-900")}>
              ...
              <TableRow className={cn("bg-emerald-50 border-t-2 border-black", isPrint ? "h-6" : "h-8")}>
                ...
                <TableCell className={cn("text-center font-black border bg-emerald-900 text-white leading-none", isPrint ? "text-[7.5px] p-0.5 border-black" : "text-sm")}>{sortedEmployees.reduce((sum, emp) => sum + calculateEmployeeHours(emp.id), 0)}</TableCell>
              </TableRow>
            </TableBody>
          </Table>
        </div>
        ...
        <Table className={cn("border-collapse border-spacing-0 table-fixed", isPrint ? "w-full border-2 border-black" : "w-full min-w-[1000px]")}>
          ...
          <TableHeader>
            <TableRow className={cn(isPrint ? "bg-slate-50/50 border-b-2 border-black h-5" : "bg-emerald-50/50 hover:bg-emerald-50/50 h-7")}>
              <TableHead className={cn("font-black text-emerald-900 border text-center sticky left-0 z-20 bg-emerald-50/50 leading-none", isPrint ? "text-[7.5px] p-0.5 border-black text-black" : "text-xs p-1 border-emerald-100")} rowSpan={2}>{isRTL ? "المعلم" : "Teacher"}</TableHead>
              {DAYS.map(day => {
              ...
              <TableHead className={cn("text-center font-black border bg-emerald-900 text-white leading-none", isPrint ? "text-[7.5px] p-0.5 border-black" : "text-xs p-1")} rowSpan={2}>{isRTL ? "المجموع" : "Total"}</TableHead>
            </TableRow>
            <TableRow className={cn(isPrint ? "bg-slate-50/20 border-b-2 border-black h-5" : "bg-emerald-50/20 h-7")}>
              {DAYS.map(day => {
                const activePeriods = activePeriodsPerDay[day.id] || [];
                return activePeriods.map(p => (
                  <TableHead key={`${day.id}-${p}`} className={cn("text-center font-bold border p-0.5 transition-colors duration-150 leading-none", isPrint ? "text-[6.5px] border-black text-black" : cn("border-emerald-100 text-slate-400", (hoveredCell?.dayId === day.id && hoveredCell?.period === p) && "bg-emerald-100 text-emerald-900"))}>
                    <span className="font-black block">{isRTL ? `ح${p}` : `P${p}`}</span>
                  </TableHead>
                ));
              })}
          ...
          <TableBody>
            {sortedEmployees.map(emp => (
              <TableRow key={emp.id} className={cn("group transition-colors duration-150", isPrint ? "h-5 border-b border-black" : "h-8 hover:bg-emerald-50/30")}>
                <TableCell className={cn("font-bold border bg-white truncate sticky left-0 z-10 shadow-sm leading-tight", isPrint ? "text-[7.5px] p-0.5 border-black text-black" : cn("text-[11px] p-1 border-emerald-100 text-emerald-950"))}>{emp.lastName} {emp.firstName}</TableCell>
                {DAYS.map(day => {
                ...
                <TableCell className={cn("text-center border bg-emerald-50 font-black leading-none", isPrint ? "text-[8px] border-black text-black" : "text-sm border-emerald-200 text-emerald-700")}>{calculateEmployeeHours(emp.id)}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
